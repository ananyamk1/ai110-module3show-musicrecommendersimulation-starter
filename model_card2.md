...
I named my model VibeCompass 1.0. I chose that name because the recommender is meant to guide a listener toward songs that match their current taste direction, not just a genre label. The name also fits the way I built the project: I kept adjusting the scoring logic until it pointed more reliably toward the kind of music I wanted it to surface.
...
I also documented the flow in a Mermaid diagram and made the CLI output a readable ASCII table so the process is easy to follow from input to ranked output. That helped me connect the math in the code with the way I present the results in the terminal.
